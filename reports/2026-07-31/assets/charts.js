...
(function() {
  ...
  c1.setOption({animation:false,tooltip:Object.assign({trigger:'axis',axisPointer:{type:'shadow'},formatter:function(p){return p[0].name+'<br/>数值：'+p[0].value;}},tooltip()),grid:{left:'8%',right:'8%',top:'8%',bottom:'20%',containLabel:true},xAxis:Object.assign({type:'category',data:['中国AI\n行业增速','Azure\n同比增速','月之暗面\n估值','微软单日\n市值增量'],axisLabel:{interval:0,color:muted,fontSize:10,lineHeight:14}},axisStyle()),yAxis:Object.assign({type:'value',name:'数值（%/十亿美元）',nameTextStyle:{color:muted}},axisStyle()),series:[{type:'bar',barWidth:'48%',data:[{value:30,itemStyle:{color:accent2}},{value:43,itemStyle:{color:accent}},{value:35,itemStyle:{color:accent3}},{value:450,itemStyle:{color:danger}}],label:{show:true,position:'top',color:ink,fontSize:11,formatter:function(p){return p.dataIndex===0||p.dataIndex===1?p.value+'%':'$'+p.value+'B';}},itemStyle:{borderRadius:[6,6,0,0]}}]});
  var c2 = echarts.init(document.getElementById('chart-crypto-moves'), null, {renderer:'svg'});
  c2.setOption({animation:false,tooltip:Object.assign({trigger:'axis',axisPointer:{type:'shadow'},formatter:function(p){return p[0].name+'<br/>24h：'+p[0].value+'%';}},tooltip()),grid:{left:'8%',right:'8%',top:'8%',bottom:'12%',containLabel:true},xAxis:Object.assign({type:'category',data:['UNI','ADA','BNB','BTC','ETH','SOL','MIM'],axisLabel:{color:muted,fontSize:11}},axisStyle()),yAxis:Object.assign({type:'value',name:'涨跌幅 (%)',nameTextStyle:{color:muted}},axisStyle()),series:[{type:'bar',barWidth:'52%',data:[13.14,3.72,3.00,1.25,0.70,-0.10,-74.00].map(function(v){return {value:v,itemStyle:{color:v>=0?accent2:danger}};}),label:{show:true,position:function(p){return p.value>=0?'top':'bottom';},color:ink,fontSize:11,formatter:'{c}%'},itemStyle:{borderRadius:[6,6,0,0]}}]});
  var c3 = echarts.init(document.getElementById('chart-market-cap'), null, {renderer:'svg'});
  c3.setOption({animation:false,color:[accent,accent2,accent3,danger,muted],tooltip:Object.assign({trigger:'item',formatter:function(p){return p.name+'<br/>市值：$'+p.value+'T ('+p.percent+'%)';}},tooltip()),legend:{bottom:'2%',left:'center',textStyle:{color:muted,fontSize:11}},series:[{type:'pie',radius:['42%','68%'],center:['50%','45%'],itemStyle:{borderColor:bg2,borderWidth:3},label:{show:true,color:ink,fontSize:11,formatter:'{b}\n{d}%'},data:[{value:1.30,name:'BTC',itemStyle:{color:accent}},{value:0.23,name:'ETH',itemStyle:{color:accent2}},{value:0.30,name:'稳定币',itemStyle:{color:accent3}},{value:0.06,name:'DeFi',itemStyle:{color:danger}},{value:0.41,name:'其他',itemStyle:{color:muted}}]}]});
